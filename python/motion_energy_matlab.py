...
-------
PreprocStep
- parent for all other steps
@run
@show 
    Unclear how to handle FIR lags
    Not all steps will have a show() function, e.g. downsample, colorspace, etc
.ID independent of data, params - useful for multiple versions of code
.ID for Params


Params
- ID for set of parameters for a particular PreprocStep
- Need different parameter values for each 
.tag for easy reference (numbered?)


Pipeline
.ID independent of data set
.ID for data set preprocessed (oStimulus in current usage)
.tag for combo
...
Notes
-----
The final structure should be able to implement ConvNets in a natural way

Consider implementing in/with skimage
- TALK TO STEPHAN VAN DER WALT

"""

class wavelets3d(object):
    """Class for wavelet preprocessing"""
    
    def __init__(self, S, dir_divisions=8, dir_selective=True, 
        sfdivisions=5, sfmax=9.0, sfmin=2.0, local_dc=0, sf_gaussratio=0.5, senv_max=0.3, std_step=2.5, 
        tfmax=3.0, tfmin=1.0, tfdivisions=5, tsize=9, zerotf=1, tf_gaussratio=0.4, tenv_max=0.3, 
        phasemode=0, phasemode_sfmax=np.inf, 
        wrap_all=0, f_step_log=0, fenv_mode=0, gaborcachemode=0,
        valid_w_index=np.NaN, zeromean=1, verbose=1):
        """
        Parameters
        ----------
        S : array, [x,y,t]
            Stimulus matrix to preprocess. The range of images will affect filter output values
            (0-1 vs 0-255), but that difference will go away if each channel is subsequently :
            z-scored or otherwise normalized. 

        show_only = If this is set to True, the function returns wavelets
        ...
        dir_divisions = Number of directions for wavelets (default: 8)
        dir_selective = a flag for whether the model should be selective
        ...
        # Phase parameters
        phasemode : scalar
            A parameter to specify how to deal with phase information 
    ...
    def run(self, show_only=False, **kwargs):
    """Preprocessing of stimuli using a Gabor wavelet basis set
    ...
    -------
    spp : array-like
        Preprocessed stimuli that can be used for STRF fitting.
        NxD matrix, N=sample size, D=dimensions (model channels)
    params : dict
        dict that contains parameters for preprocessing, with additional fields:
        ...
        zeromean_value = Offset value of total movie. This is set if `zeromean` input is non-zero.
    ...
    # Run preprocessing of stimuli 
    spp, params = wavelets3d(S, PARAMS)

    SEE ALSO: make3dgabor, preprocSpectra
    """
    #class = 'preprocWavelets_grid';
    # Misc fill-in
    ...
    if length(stimxytsize) == 2:
        ...
        if numel(S)>100*100*100000 :
            ...
            for ci=1:length(nColorChannels)
                [tstim p] = feval(class, sstr{ci}, params);
                spp = cat(2,spp, tstim);
                if verbose:
            ...
            spp = [];
            for ci=1:nColorChannels
                [tstim p] = feval(class, S(:,:,:,ci), params);
                spp = cat(2,spp, tstim);
        params = p;
        varargout{1} = spp;
        if nargout >1:
    ...
    patchxytsize = [stimxytsize(1:2) tsize];
    ...
    if show_only:
        if zeromean:
            ...
            if isfield(params, 'zeromean_value'):
                S = bsxfun(@minus, S, zeromean_value);
            else:
                thismean = mean(S(:));
                S = bsxfun(@minus, S, thismean);
                zeromean_value = thismean;
            
        


    # Make a list of gabor parameters
    if ~isfield(params,'gaborparams') || phasemode == 5 || ...:
            phasemode == 7
    ...
    if verbose && any(valid_w_index):
        fprintf('Valid channel num: %d\n', length(valid_w_index));
    ...
    if show_only:
        ...
        spp = zeros(stimxytsize(3), waveletchannelnum, 'single');
    else:
        if verbose, disp('Making wavelets...'); :
        if ~any(valid_w_index):
            gnum = length(waveletchannelnum);
        else:
            gnum = length(valid_w_index);
    ...
    if gaborcachemode==1:
        ...
        gtwcache = zeros([2 tsize waveletchannelnum], 'single');
    ...
    for ii=1:waveletchannelnum
        ...
        if any(valid_w_index) && ~any(ii==valid_w_index), continue, :
    ...
    if verbose:
        ...
        if show_only:
            disp(sprintf('%d channels, %d samples', size(spp,2), size(spp,1)));
        else:
    ...
    if show_only:
        if phasemode==5 || phasemode==6 || phasemode==7 ...:
                || phasemode==8
            ...
            if phasemode==5 || phasemode==7 # return dPhase/dt channels only:
                spp = spp(:,pind);
...
def getGaborParameters(params,aspect_ratio):
    """Get gabor wavelet parameters"""

    if f_step_log:
        sf_array = logspace(log10(sfmin), log10(sfmax), sfdivisions);
        if zerotf:
            tf_array = logspace(log10(tfmin), log10(tfmax), tfdivisions-1);
            tf_array = [0 tf_array];
        else:
            tf_array = logspace(log10(tfmin), log10(tfmax), tfdivisions);
        
    else:
        sf_array = linspace(sfmin, sfmax, sfdivisions);
        tf_array = linspace(tfmin, tfmax, tfdivisions);
        
    

    dir_array = (0:dir_divisions-1)/dir_divisions*360;
    ...
    for ti=1:tfdivisions
        tf = tf_array(ti);
        for fi = 1:sfdivisions
            sf = sf_array(fi);
            
            if fenv_mode:
                f = sqrt(sf.^2+tf.^2);
                fenv = min([fenv_max 1/f*f_gaussratio]);
                tenv = fenv;
                senv = fenv;
            else:
                senv = senv_max;
                if sf ~= 0:
                    senv = min([senv_max 1/sf*sf_gaussratio]);
                
                tenv = tenv_max;
                if tf ~= 0:
                    tenv = min([tenv_max 1/tf*tf_gaussratio]);
            ...
            if aspect_ratio==1:
                ...
                centers = senv*std_step*(-numsps2:numsps2) + 0.5;
