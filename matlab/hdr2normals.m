function N = hdr2normals(hdrIm,AdjustToBlenderWorld)
% Usage: N = hdr2normals(hdrIm,AdjustToBlenderWorld)
% 
% Gets and properly normalizes normal vectors from an HDR image. Normals
% can have negative values, but none of the image formats output by Blender
% store negative values; instead, we store a high dynamic range (hdr)
...
% available).  
% 
% hdrIm can be numeric (read in via hdrread) or a character file string
% (for an .hdr file). 
% 
