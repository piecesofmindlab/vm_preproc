function [stim, params] = preprocTest(rawStim, params)
% function [stim, params] = preprocWavelets_grid(rawStim, params);
%
...
%             rawStim = bsxfun(@minus, rawStim, params.zeromean_value);
% 		else
			thismean = mean(rawStim(:));
%             rawStim = bsxfun(@minus, rawStim, thismean);
...
for ii=1:waveletchannelnum
	...
	if ~thesame
        if params.gaborcachemode==2
		    ...
		    [gabor0 gabor90 gtw] = make3dgabor_frames(patchxytsize, [thisgparam(1:end-1); 0]);
...
if ~isfield(params, 'sfdivisions')
	 params.sfdivisions = 5;
end

if ~isfield(params, 'tfdivisions')
	 params.tfdivisions = 5;
end
...
if ~isfield(params, 'tfmin')
     params.tfmin = 1.0;
end

if ~isfield(params, 'zerotf')
     params.zerotf = 1;
end
