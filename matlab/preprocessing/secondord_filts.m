function [filts] = secondord_filts(strf,filtidxs)
...
if strf.interactIdx.covtime
    ...
    delays = double(length(strf.delays));
end
...
cnt=1;
for ii=filtidxs
    filts(:,:,:,cnt) = reshape(u(:,ii,:), [sqrt(strf.interactIdx.covsz) sqrt(strf.interactIdx.covsz) delays]);
    cnt=cnt+1;
end
