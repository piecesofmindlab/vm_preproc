function [filts] = preprocRTAC_filts(strf, filtidxs)
...
if strf.params.RTAC(2) && ~strf.params.RTAC(1)
    
    filts = zeros([sqrt(strf.params.covsz) sqrt(strf.params.covsz) delays size(filtidxs,2)]);
    
    cnt=1;
    for ii=filtidxs
        filts(:,:,:,cnt) = reshape(u(:,ii,:), [sqrt(strf.params.covsz) sqrt(strf.params.covsz) delays]);
        cnt=cnt+1;
    end
    
elseif strf.params.RTAC(1) && strf.params.RTAC(2) 
    ...
    filts = reshape(RTA, [sqrt(strf.params.covsz) sqrt(strf.params.covsz) delays]);
end
