...
for ii=filtidxs
    ...
    ff = mean(reshape(ff, [strf.params.covsz delays]),1);
    [m, idx] = max(ff);
    ff2 = filts(:,:,idx,cnt);
    fit = fitA_gabors(ff2(:));
    filts(:,:,idx,cnt) = reshape(fit2A(fit, sqrt(strf.params.covsz)), sqrt(strf.params.covsz), sqrt(strf.params.covsz));
    
    params = loadGaborfitParams;
    for dd = setdiff(1:delays, idx)
        ff = filts(:,:,dd,cnt);
        % fit = fitA_gabors(ff(:));
        s0 = max(abs(ff(:)));
        fit.n = sqrt(strf.params.covsz);
        [fit2,subcount,err,snr] = gabor_phasefit2(ff,fit,params);
        fit2.s =  s0;
        filts(:,:,dd,cnt) = reshape(fit2A(fit2, sqrt(strf.params.covsz)), sqrt(strf.params.covsz), sqrt(strf.params.covsz));
    end
    cnt=cnt+1;
end
