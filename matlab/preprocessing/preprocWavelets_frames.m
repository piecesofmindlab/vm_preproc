function [stim, params] = preprocWavelets_frames(rawStim, params)
...
%%% a special high-speed mode
if exist('params', 'var') && isfield(params,'gaborchahemode') && params.gaborcachemode==3
    stim = calcAtOnce(rawStim, params);
    return
end
...
for ii=1:waveletchannelnum
	...
	if ~thesame
        if params.gaborcachemode==2
            ...
            gp = [thisgparam(1:end-1); 0; params.elong];
		    [gabor0 gabor90 gtw] = make3dgabor_frames(patchxytsize, gp);
	...
	if params.show_or_preprocess
		if ~thesame
			...
			if senv<maskenv_below
				%tmask = find(max(abs(gabor0)+abs(gabor90),[],2)> ...
				%			 masklimit);
                 smask = find(sum(abs(gabors),1)>masklimit);
                [chout0 chout90] = dotdelay_frames(gabors(:,smask), gtw, rawStim(smask,:));
   			else
                [chout0 chout90] = dotdelay_frames(gabors, gtw, rawStim);
			end
		end
		switch phaseparam
		case 0
...
%            rgs = gabors(:,1)*gtw(2,:)+gabors(:,2)*gtw(1,:);
            ...
            rgs=reshape(rgs, patchxytsize);%[imsize tsize]);
			gaborbank(:,:,:,wcount) = rgs;
		case {0,2,5,6,7,8,9}
            % reconstruct space-time Gabor
            %rgc = -gabors(:,1)*gtw(1,:)+gabors(:,2)*gtw(2,:);
            rgc = -gabors(1,:)'*gtw(1,:)+gabors(2,:)'*gtw(2,:);
            rgc=reshape(rgc, patchxytsize);%[imsize tsize]);
...
params.class = 'preprocWavelets_frames';
...
if isreal(params.dirdivisions)
  dir_array = (0:params.dirdivisions-1)/params.dirdivisions*360;
else
  % a special case: use DOG instead of gabors
  dir_array = [-i i];
end
...
function stim = calcAtOnce(rawStim, params)
% %    gaborcache = zeros([2 prod(patchxytsize(1:2)) waveletchannelnum], 'single');
%     gtwcache = zeros([2 params.tsize waveletchannelnum], 'single');
% 
% 	        gabors = params.gaborcache(:,:,ii);
%             gtw = params.gtwcache(:,:,ii);
% 
% gabors = reshape(params.gaborcache, [], 
