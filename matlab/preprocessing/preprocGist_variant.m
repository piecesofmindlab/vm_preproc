function [Spreproc, params] = preprocGist_variant(S,params)
% Usage: [Spreproc, params] = preprocGist_variant(S,params)
% 
...
% Parameters
% ----------
% S : 3D image matrix (X,Y,Time)
%   Stack of images to be processed; should be (X, Y, Time), should also be
%   luminance images - process/remove color before getting here! 
% params : struct array
%   preprocessing parameter struct array, with fields:
%   .imageSize = nan
...
% Default parameters
dparam.imageSize = nan;
dparam.orientationsPerScale = [8 8 8 8];
dparam.numberBlocks = 4;
dparam.fc_prefilt = 4;
dparam.boundaryExtension = 32; % number of pixels to pad

% Fill in defaults / inputs 
params = defaultOpt(params,dparam);

% resize and crop image to make it square
if isnan(params.imageSize)
    params.imageSize = [size(S,1) size(S,2)];
    img = S;
else
    img = imresizecrop(S, params.imageSize, 'bilinear');
end

% Define Gabors
params.G = createGabor(params.orientationsPerScale, params.imageSize+2*params.boundaryExtension);

% Precompute number of filter transfer functions 
%Nfeatures = size(params.G,3)*params.numberBlocks^2;
%Spreproc = zeros([Nscenes Nfeatures], 'single');

% scale intensities to be in the range [0 255]
% Necessary? For whatever reason, changing the range from 0-1 to
% 0-255 seems to increase the contrast of the (pre-)filtered image.
% The correlation between outputs of the two is .97 (not perfect,
% but close...
%img = img-min(img(:));
%img = 255*img/max(img(:));
img = double(img)/255;
% prefiltering: local contrast scaling
%output    = prefilt(img, params.fc_prefilt);
% compute gist:
Spreproc = gistGabor(img, params)';
...
%   params.w = number of windows (w*w)
%   params.G = precomputed transfer functions
%
% Output:
...
w = params.numberBlocks;
G = params.G;
be = params.boundaryExtension;
switch ndims(img)
    case {2,3}
