...
params.covsz = size(rawStim,1)*size(rawStim,2)*size(rawStim,3);
...
if params.RTAC(1) & params.RTAC(2)
    [A, idxr, idxc] = getSubMatrix2([size(rawStim,1) size(rawStim,2) size(rawStim,3) (params.covdelays + 1)], params.locality*2 +1, [1 1 1 params.covtime]);
    params.idxr = idxr; params.idxc = idxc;
    rawStim = reshape(rawStim, size(rawStim,1)*size(rawStim,2)*size(rawStim,3), size(rawStim,4));
...
elseif ~params.RTAC(1) & params.RTAC(2)
    [A, idxr, idxc] = getSubMatrix2([size(rawStim,1) size(rawStim,2) size(rawStim,3) (params.covdelays + 1)], params.locality*2 +1, [1 1 params.covtime]);
    rawStim = reshape(rawStim, size(rawStim,1)*size(rawStim,2), size(rawStim,3));
