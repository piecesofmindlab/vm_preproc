...
if strf.params.RTAC(1)
    RTA = strf.w1(1:(strf.params.delays + 1)*strf.params.covsz,:);
    lines = 1;
    covstart = (strf.params.delays + 1)*strf.params.covsz+1;
else
...
if strf.params.RTAC(2)
    ...
    if strf.params.covtime
        idx2 =sub2ind([strf.params.covsz*(strf.params.delays + 1) strf.params.covsz*(strf.params.delays + 1)], strf.params.idxr, strf.params.idxc);
        RTC = squeeze(zeros([strf.params.covsz*(strf.params.delays + 1) strf.params.covsz*(strf.params.delays + 1)]));
        RTC(idx2) = RTCsub;
        %[u,s,v] = svd(RTC);
        [u,s] = eig(RTC);
        delays = (strf.params.delays + 1);
    else
...
filtRTC1 = reshape(u(:,end,:), [sqrt(strf.params.covsz) sqrt(strf.params.covsz) delays]);
filtRTC2 = reshape(u(:,end-1,:), [sqrt(strf.params.covsz) sqrt(strf.params.covsz) delays]);
...
RTAmax = max(abs(filtRTA(:)));
RTC1max = max(abs(filtRTC1(:)));
RTC2max = max(abs(filtRTC2(:)));
f2 = figure(3);
