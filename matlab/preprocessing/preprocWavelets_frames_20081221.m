...
masklimit = 0.01;   %% pixel value < masklimit AND
maskenv_below = 0.1; % spatial envelope < maskenv_below x stimulus size

if params.gaborcachemode==1
    gaborcache = zeros([prod(patchxytsize(1:2)) waveletchannelnum*2], 'single');
	...
	if params.show_or_preprocess
		if ~thesame
			...
			if senv<maskenv_below
				tmask = find(max(abs(gabor0)+abs(gabor90),[],2)>masklimit);
                [chout0 chout90] = dotdelay_frames(gabor0(tmask), gabor90(tmask), gtw, rawStim(tmask,:));
   			else
                [chout0 chout90] = dotdelay_frames(gabor0, gabor90, gtw, rawStim);
			end
		end
		switch phaseparam
		case 0
		...
		case {1,3,4}
            ...
            rgs = gabor0*gtw(2,:)+gabor90*gtw(1,:);
            rgs=reshape(rgs, [imsize tsize]);
			gaborbank(:,:,:,wcount) = rgs;
		case {0,2,5,6,7,8,9}
            % reconstruct space-time Gabor
            rgc = -gabor0*gtw(1,:)+gabor90*gtw(2,:);
            rgc=reshape(rgc, [imsize tsize]);
