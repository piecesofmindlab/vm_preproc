function [filtRTA, filtRTC1, filtRTC2, eigenvalues] = secondord_aud_vis(strf,filtidxs, maxeig)
...
if strf.interactIdx.covtime
    ...
    delays = double(length(strf.delays1));
end

lines = size(filtidxs,2)+1;
covstart = 1;
...
filts(:,:,1) = squeeze(strf.w1);
maxs(1) = max([squeeze(max(max(abs(filts(:,:,1))))); 1e-6]);
cnt=2;
for ii=filtidxs
    filts(:,:,cnt) = reshape(u(:,ii,:), [strf.interactIdx.covsz delays]);
...
for ff = 1:size(filtidxs,2)+1
        subplot(1,lines,ff); imagesc(filts(:,:,ff), [-maxs(ff) maxs(ff)]); axis image; axis off; 
            if ff == 1
                title(['RTA']);
            else
            title([sprintf('RTC%s', num2str(filtidxs(ff-1)))]);
            end
end
