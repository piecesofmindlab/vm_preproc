function [filts] = preprocRTAC_filt_vis(strf, filtidxs, normalize)
...
for ii = 1:size(w2,1)
    ...
    elseif w2(wsidx(ii),1) < w2(wsidx(ii),2)
        w3(wsidx(ii),1) = w2(wsidx(ii),2);
        w3(wsidx(ii),2) = w2(wsidx(ii),1);
        ws(ii) = -ws(ii);
...
if normalize
    ...
    for ff = 1:lines
        for dd = 1:delays
            subplot(lines,delays+1,(ff-1)*(delays+1)+dd); imagesc(sign(ws(ff)).*filters2(:,:,dd,wsidx(ff)), [-maxf maxf]); axis image; axis off;  colormap gray;
                % if ff == 1
                %     title(['RTA: ' num2str(-dd+1)]);
                % else
                %     title([sprintf('RTC%s', num2str(filtidxs(ff-1))) ': ' num2str(-dd+1)]);
        ...
        plot([-1 1], [0 0])
        hold on; plot([0 0], [-1 1])
        hold on; plot([0 1], [0 w3(wsidx(ff),1)/abs(ws(ff))], 'LineWidth', 4);
        hold on; plot([0 -1], [0 w3(wsidx(ff),2)/abs(ws(ff))], 'LineWidth', 4); axis image;
        
        set(gca, 'color', [1 1-abs(ws(ff)) 1-abs(ws(ff))])
