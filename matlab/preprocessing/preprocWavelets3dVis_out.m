function [out, strf] = preprocWavelets3dVis(strf, fnamec, visopt)
...
% fnamec = strcat('/auto/k5/moliver/u001mat/u001_3_11_strf.gif');
%--------------------------
% show movie
%--------------------------
...
for ii=1:vis_t_size*moviereps
	...
	subplot(2,2,3), imagesc(im_spec_ex(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	title(sprintf('ex complex: %.3f', maxim_spec_ex/totalmax));

	subplot(2,2,4), imagesc(im_spec_sp(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	...
	figure(7);
	I6 =getframe(gcf);
	imwrite(rgb2ind(I6.cdata, gray), gray, [fnamec '_mov.gif'], 'gif', 'DelayTime', delaysec);
	elseif ii <= vis_t_size*moviereps
	figure(7);
	I6 =getframe(gcf);
	imwrite(rgb2ind(I6.cdata, gray), gray, [fnamec '_mov.gif'], 'gif', 'WriteMode', 'append', 'DelayTime', delaysec)
	end
end

figure(6);
I9 =getframe(gcf);
imwrite(rgb2ind(I9.cdata, colorcube), colorcube, [fnamec '_RF.gif'], 'gif')
strf.params = params;
