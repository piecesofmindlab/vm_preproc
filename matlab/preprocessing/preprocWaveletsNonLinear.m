...
if ~isfield(params, 'gainControlOut')
     params.gainControlOut = [];
end
...
if params.show_or_preprocess
    if ~isempty(params.gainControl)
        ...
        if ifstr(S), S = refmat_recover(S); end
        S = single(S);
...
if isfield(params, 'wclass')
  ...
  if params.verbose, disp('Calling preprocWavelets_frames...');end
  [PS, params] = preprocWavelets_frames(S, params);
end
...
if params.show_or_preprocess    
    if ~isempty(params.gainControlOut)
        ...
        if params.verbose, fprintf('Processing gain controls for output...\n'); end
        for ii=1:size(PS,2)
    ...
    if ischar(params.nonLinOutExp)
        switch params.nonLinOutExp
            ...
            case {'logstd'}
                if isfield(params,'nonLinOutStd')
                    ...
                    stds = nanstd(PS);
                    parms.nonLinOutStd = stds;
                end
                d = params.nonLinOutParam;
                for ii=1:size(PS,2)
                    PS(:,ii) = log(d+PS(:,ii)/stds(ii));
                end
            case {'logmean'}
                if isfield(params,'nonLinOutMean')
                    ...
                    means = nanmean(PS);
                    parms.nonLinOutStd = means;
                end
                d = params.nonLinOutParam;
                for ii=1:size(PS,2)
                    PS(:,ii) = log(d+PS(:,ii)/means(ii));
                end
            case {'linear'}
        ...
        if length(params.nonLinOutExp) == 1
            ...
            for ii=1:length(params.nonLinOutExp)
                Pt = cat(2, Pt, abs(PS).^params.nonLinOutExp(ii).*sign(PS));
            end
            PS = Pt;
        end
    end
