function varargout = dbCheck(ModStruct,STRFdb,dbName,Exclude,View)
% Usage: [ModStruct [,IsNewModel][,OldModel]] = dbCheck(ModStruct,STRFdb,dbName,Exclude,View)
% 
...
%   View = Cell array for name of view to use to speed up comparisons
%       for fMRI models: use {'Mod','ModelName'};
%       for model params predicting model params, use
%           {'ModY_All','ModY_All'}
%       for strf simulations, use {'ModelType','ModelType'}
% 
% Outputs: 
...
if isempty(View)
    c = couch(c,'all_docs');
    Models = c.db.all_docs.rows;
else
    % Speed-up search w/ view
    c.design_name = View{1};
    disp('Querying database')
    docs = couch(c,'get view',View{2});
    Models = {};
    for ii = 1:length(docs.rows);
        Models = [Models,docs.rows(ii).key];
    end
    if strcmp(View{1},'Mod')
        % Search by model name
        idx = ismember(Models,ModStruct.Mod);
        Models = docs.rows(idx);
    elseif strcmp(View{1},'ModY_All')
        % Search by model x predicting model y
        idx = ismember(Models,ModStruct.ModelY);
        Models = docs.rows(idx);
    elseif strcmp(View{1},'ModelType')
        % Search by model type
        idx = ismember(Models,ModStruct.Type);
        Models = docs.rows(idx);
    end
end
O = ModStruct;
for iE = 1:length(Exclude);
    if isfield(O,Exclude{iE})
        O = rmfield(O,Exclude{iE});
    end
end
disp('Comparing models returned to query struct: ');
disp(O);
NewModel = true;
ModOut = {};
Ct = 1; % Model count
for iR = 1:length(Models);
    %if compareStruct(O,Models(iR).value) % Old, compatible with json2mat/mat2json
    if compareStruct(O,Models(iR).value) && Ct==1
        fprintf('Model found: \n%10s : %s\n%10s : %s\n%10s : %s\n',...
            'id',Models(iR).value.([prefix '_id']),'rev',Models(iR).value.([prefix '_rev']),'path',Models(iR).value.Path);
        try
            % A little more info, if available:
            fprintf('%10s : %s\n%10s : %s\n',...
                'ValDat',Models(iR).value.ValDat,'NoisePreds',Models(iR).value.NoisePreds{:});
        end
        fprintf('\n\n')
        ModStruct.Path = Models(iR).value.Path;
        ModStruct.([prefix '_id']) = Models(iR).value.([prefix '_id']);
        ModStruct.([prefix '_rev']) = Models(iR).value.([prefix '_rev']);
        ModOut{Ct} = Models(iR).value;
        NewModel = false;
        Ct = Ct+1;
    elseif compareStruct(O,Models(iR).value) && Ct>1
        fprintf('Model found: \n%10s : %s\n%10s : %s\n%10s : %s\n',...
            'id',Models(iR).value.([prefix '_id']),'rev',Models(iR).value.([prefix '_rev']),'path',Models(iR).value.Path);
        try
            % A little more info, if available:
...
if NewModel
    fprintf('Model not found - creating new model!\n\n')
    [s,UUID] = system('uuidgen');
    uuid = strrep(lower(UUID(1:end-1)),'-',''); % end-1 because the system adds a newline character
    % Hacky shit - file names will be 100% unintelligible without .Mod
    % and .SubID fields present in ModStruct.
    ...
    ModStruct.([prefix '_id']) = uuid; % "x" is removed by couch_curl.m before being uploaded
    ModOut = [];
end
% Outputs
