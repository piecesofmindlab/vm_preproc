function [interactIdx, interactDelay, A] = getInteractIdx(rawStim, delays, locality, covtime)

% Make indicies of top of matrix
[A, idxr, idxc] = getSubMatrix([size(rawStim,1) size(rawStim,2) (max(delays) + 1)], locality*2 +1, [1 1 covtime]);
...
if covtime
    ...
    interactIdx.covsz = double(size(rawStim,1)*size(rawStim,2));
